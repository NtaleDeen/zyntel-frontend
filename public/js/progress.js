// progress.js - Refactored to use a centralized auth module and improved search/pagination.
...
// ----------------------------------------------------
const API_URL = "https://zyntel-data-updater.onrender.com/api/progress";
...
/**
 * Fetches progress data from the API and calls the render function.
 */
async function fetchprogressData() {
    showLoadingSpinner();
    progressBody.innerHTML = `<tr><td colspan="4" class="text-center py-4 text-gray-500">Loading data...</td></tr>`;
    progressMessage.classList.add('hidden');

    const token = getToken();
    if (!token) {
        showMessage(progressMessage, 'Authentication required. Please log in.', 'error');
        progressBody.innerHTML = `<tr><td colspan="4" class="text-center py-4 text-red-500">Authentication failed.</td></tr>`;
        hideLoadingSpinner();
        return;
    }
    ...
    try {
        ...
        if (!Array.isArray(allprogressData) || allprogressData.length === 0) {
            progressBody.innerHTML = `<tr><td colspan="4" class="text-center py-4 text-gray-500">No data found.</td></tr>`;
        ...
        showMessage(progressMessage, `Failed to load data: ${error.message}. Please check the backend API.`, 'error');
        progressBody.innerHTML = `<tr><td colspan="4" class="text-center py-4 text-red-500">Error loading data.</td></tr>`;
    } finally {
        hideLoadingSpinner();
    }
}
...
/**
 * Renders the fetched progressdata into the table with pagination.
 */
function renderprogress(data) {
    ...
    if (paginatedData.length === 0) {
        progressBody.innerHTML = `<tr><td colspan="4" class="text-center py-4 text-gray-500">No matching data found.</td></tr>`;
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (searchInput) {
        searchInput.addEventListener('input', (event) => {
            ...
            renderprogress(allprogressData); // Re-render the data with the new search query
        });
    }
});